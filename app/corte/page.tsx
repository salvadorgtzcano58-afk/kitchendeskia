...
type Pedido = {
  ...
  hora: string
  estado: 'preparando' | 'listo' | 'entregado'
}
...
export default function CorteTurnoPage() {
  ...
  const [nuevoPedido, setNuevoPedido] = useState<{canal:Canal; items:string; total:string; metodo_pago:MetodoPago}>({
    canal:'whatsapp', items:'', total:'', metodo_pago:'efectivo'
  })
  ...
  const agregarPedido = () => {
    if (!nuevoPedido.items.trim() || !nuevoPedido.total) return
    const p: Pedido = {
      id: Date.now(),
    ...
    setNuevoPedido({ canal:'whatsapp', items:'', total:'', metodo_pago:'efectivo' })
  ...
  const eliminarGasto = (i: number) => {
    setGastos(prev => prev.filter((_, idx) => idx !== i))
  }

  // cuando cambia el canal, resetea el método de pago al primero disponible
  const cambiarCanal = (canal: Canal) => {
    const metodos = PAGOS_POR_CANAL[canal]
    setNuevoPedido(p => ({ ...p, canal, metodo_pago: metodos[0] }))
  }
      ...
      <main style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
        ...
        {!turnoActivo && (
          <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:16 }}>
            ...
            <div style={{ fontSize:18, fontWeight:700, color:'var(--text)' }}>Sin turno activo</div>
                      ...
                      return (
                        <div key={v.canal}>
                          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
                            ...
                            <span style={{ fontSize:13, fontWeight:600, color:'var(--text)' }}>${v.total.toLocaleString()}</span>
                          </div>
                    ...
                    {pedidos.map(p => {
                      ...
                      return (
                        <div key={p.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 10px', background:'var(--surface2)', borderRadius:8 }}>
                          ...
                          <span style={{ fontSize:10, color:'var(--text3)', minWidth:80 }}>{PAGO_LABEL[p.metodo_pago]}</span>
                          <span style={{ fontSize:13, fontWeight:600, color:'var(--accent)', minWidth:50, textAlign:'right' }}>${p.total}</span>
                          <button onClick={() => eliminarPedido(p.id)} style={{ background:'none', border:'none', color:'var(--text3)', cursor:'pointer', fontSize:14, padding:'0 4px' }}>✕</button>
                        </div>
                      )
                    })}
                  </div>
                ...
                {gastos.length === 0 ? (
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                    ...
                    <div style={{ display:'flex', justifyContent:'space-between', fontSize:13, fontWeight:600, marginTop:6 }}>
                      <span style={{ color:'var(--text)' }}>Total gastos</span>
        ...
        {showNuevoPedido && (
          <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.7)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:100 }}>
            <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:14, padding:24, width:400 }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                  ...
                  <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                    {(Object.keys(CANAL_INFO) as Canal[]).map(c => (
                      <button key={c} onClick={() => cambiarCanal(c)}
                        style={{ padding:'5px 10px', borderRadius:6, fontSize:11, cursor:'pointer', fontWeight:500, background: nuevoPedido.canal === c ? CANAL_INFO[c].bg : 'var(--surface2)', color: nuevoPedido.canal === c ? CANAL_INFO[c].color : 'var(--text3)', border: nuevoPedido.canal === c ? `1px solid ${CANAL_INFO[c].color}` : '1px solid var(--border)' }}>
                        {CANAL_INFO[c].label}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div style={{ display:'flex', gap:6 }}>
                    {PAGOS_POR_CANAL[nuevoPedido.canal].map(m => (
                      <button key={m} onClick={() => setNuevoPedido(p => ({...p, metodo_pago:m}))}
                        style={{ padding:'5px 10px', borderRadius:6, fontSize:11, cursor:'pointer', fontWeight:500, background: nuevoPedido.metodo_pago === m ? 'rgba(200,241,53,0.12)' : 'var(--surface2)', color: nuevoPedido.metodo_pago === m ? 'var(--accent)' : 'var(--text3)', border: nuevoPedido.metodo_pago === m ? '1px solid rgba(200,241,53,0.4)' : '1px solid var(--border)' }}>
                        {PAGO_LABEL[m]}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  <div style={{ fontSize:10, color:'var(--text3)', marginBottom:6, textTransform:'uppercase', letterSpacing:1 }}>Producto(s)</div>
                  <input value={nuevoPedido.items} onChange={e => setNuevoPedido(p => ({...p, items:e.target.value}))}
                    placeholder="Ej: Paneki Familiar x2, Combo 2 pa 2..."
                    style={{ width:'100%', background:'var(--surface2)', border:'1px solid var(--border)', borderRadius:8, padding:'8px 12px', color:'var(--text)', fontSize:12, outline:'none', boxSizing:'border-box' }} />
                </div>
                <div>
                  <div style={{ fontSize:10, color:'var(--text3)', marginBottom:6, textTransform:'uppercase', letterSpacing:1 }}>Total ($)</div>
                  <input value={nuevoPedido.total} onChange={e => setNuevoPedido(p => ({...p, total:e.target.value}))}
                    onKeyDown={e => e.key === 'Enter' && agregarPedido()}
                ...
                <div style={{ display:'flex', gap:8, marginTop:4 }}>
                  ...
                  <button onClick={agregarPedido}
                    style={{ flex:1, padding:'10px', background:'var(--accent)', border:'none', borderRadius:8, cursor:'pointer', color:'#0e0f0c', fontSize:12, fontWeight:700 }}>
                    Registrar
                  </button>
                </div>
              </div>
        ...
        {showCierre && (
          <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.7)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:100 }}>
            <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:14, padding:24, width:360 }}>
              ...
              <div style={{ fontSize:12, color:'var(--text3)', marginBottom:16 }}>Se guardará el resumen del turno con los siguientes números:</div>
              {[
